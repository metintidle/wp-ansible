 ...
 *   WP_CLEAN_EXCLUDE_EXTENSIONS     Default mp4 — comma-separated extensions to skip
 *
 * Only PNG, JPG, WebP, and video attachments are eligible (minus excluded extensions).
 *
...
function wp_clean_excluded_extensions(): array {
    static $excluded = null;
    if ($excluded === null) {
        $raw = getenv('WP_CLEAN_EXCLUDE_EXTENSIONS') ?: 'mp4';
...
function wp_clean_is_excluded_path(string $path): bool {
    $ext = strtolower(pathinfo($path, PATHINFO_EXTENSION));
    return $ext !== '' && in_array($ext, wp_clean_excluded_extensions(), true);
}

function wp_clean_is_eligible_path(string $path): bool {
    if (wp_clean_is_excluded_path($path)) {
        return false;
    }
    $ext = strtolower(pathinfo($path, PATHINFO_EXTENSION));
...
function wp_clean_is_eligible_attachment(int $id): bool {
    ...
    if (is_string($rel) && $rel !== '' && wp_clean_is_excluded_path($rel)) {
        return false;
    }
...
$manifest = [
    ...
    'exclude_extensions' => wp_clean_excluded_extensions(),
