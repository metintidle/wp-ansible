...
#   ./run-unused-media-cleanup.sh fairfeild         # remote via SSH (ohara config)
#
...
export WP_CLEAN_EXCLUDE_EXTENSIONS="${WP_CLEAN_EXCLUDE_EXTENSIONS:-mp4}"
...
run_cleanup() {
  ...
  log "Start unused-media cleanup (dry_run=$WP_CLEAN_DRY_RUN exclude=$WP_CLEAN_EXCLUDE_EXTENSIONS) wp_root=$wp_root"
...
apply_remote() {
  ...
  ssh -F "$ssh_config" "$host" \
    "WP_CLEAN_DRY_RUN=${WP_CLEAN_DRY_RUN:-0} WP_CLEAN_EXCLUDE_EXTENSIONS=${WP_CLEAN_EXCLUDE_EXTENSIONS:-mp4} ${WRAPPER_REMOTE:-/home/ec2-user/bin/run-unused-media-cleanup.sh}"
}
